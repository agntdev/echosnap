# fix-546f7e0f2fd761de — Extraneous `/ping` and `/help` commands contradict the spec

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ECHO

The spec (`docs/spec.md`) defines only two commands: `/start` and `/echo`. `src/handlers/T01.ts` adds `/ping` and `src/handlers/T03.ts` adds `/help`, both with declared commands in `tests/commands/T01.json` and `tests/commands/T03.json`. The `/help` output in `T03.ts:9-12` also references `/ping`. These undocumented commands add scope creep and the `/help` listing is misleading — it omits `/echo` and `/div` which are actually handled.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-546f7e0f2fd761de.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-546f7e0f2fd761de.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-546f7e0f2fd761de.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
