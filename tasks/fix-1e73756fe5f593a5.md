# fix-1e73756fe5f593a5 — `/echo` truncation at 4096 chars is implemented but untested

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ECHO

`src/handlers/E1T1.ts:19-21` correctly truncates text exceeding `MAX_LENGTH` (4096) and appends `"..."`, matching the spec. However, `tests/specs/E1T1.json` has no spec exercising this path — it tests no-argument, plain echo, leading whitespace trimming, and trailing whitespace preservation, but not truncation. The truncation behavior is untested and could regress silently.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-1e73756fe5f593a5.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-1e73756fe5f593a5.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-1e73756fe5f593a5.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
