# E1T1 — /echo command with argument handling

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ECHO

Implement the /echo command in src/handlers/echo.ts. The command should:
- Reply with the provided text if present.
- Trim leading whitespace from the argument.
- If no text is provided, reply with 'Usage: /echo <text>'.
- If the text exceeds 4096 characters, truncate and append '...'.
- Write a BotSpec test in tests/specs/echo.json and register the command in tests/commands.json.
