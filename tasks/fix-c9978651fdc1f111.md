# fix-c9978651fdc1f111 — /echo with whitespace-only argument replies with empty text

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ECHO

When a user sends `/echo ` (space after command), `ctx.match` is `" "` — a non-empty string that passes the falsy `!text` check. After `text.replace(/^ +/, "")` the text becomes `""`, and `ctx.reply("")` triggers a Telegram API error (`Bad Request: message text is empty`). The empty-check must happen **after** trimming, not before. Fix: trim first, then check `if (!text)`.

Repro: send `/echo ` (single space) → no response + error logged by `bot.catch()`.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-c9978651fdc1f111.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-c9978651fdc1f111.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-c9978651fdc1f111.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
